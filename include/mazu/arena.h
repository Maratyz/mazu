...
{
    return (struct arena) {.beg = ba.dat, .end = ba.dat + ba.len};
}
...
static inline void *arena_alloc_aligned(struct arena *arn, sz n_bytes, sz align)
{
    ...
    sz padding = (sz) (-(uptr) arn->beg & ((uptr) align - 1));
...
static inline void *arena_alloc_aligned_array(struct arena *arn,
                                              sz n,
                                              sz size,
                                              sz align)
{
...
/* Allocates 'n * size' bytes out of the arena. Will return 'NULL' if the
...
{
    return str_buf_new(arena_alloc_array(arn, cap, sizeof(char)), 0, cap);
}
