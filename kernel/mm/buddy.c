/* SPDX-License-Identifier: MIT */
#include <mazu/buddy.h>

/* The algorithms in this implementation of the buddy system are from Donald
 * Knuths' The Art of Computer Programming, Volume 1. See 2.5.
 *
 * The buddy system manages memory in page-sized blocks, with allocations
 * increasing in sizes that are powers of two. These sizes are expressed using
 * base-two exponents, referred to as the "order" of an allocation. The core
 * logic for allocation and deallocation resides in 'buddy_[alloc|free]_raw',
 * which operates on orders directly. The 'buddy[alloc|free]' interface provides
 * a higher-level abstraction, converting byte-based sizes into the appropriate
 * orders before invoking the raw functions.
 */

/* Returns the smallest power of two greater than or equal to the given number.
 */
static inline sz min_power_of_two_geq(sz n)
{
    if (n == 0)
        return 1;
    if ((n & (n - 1)) == 0)
        return n; /* Already a power of two */
    for (sz shift = 1; shift < sizeof(n) * BYTE_WIDTH; shift <<= 1)
        n |= n >> shift;
    return n + 1;
}

/* Returns the largest power of two less than or equal to the given number. */
static inline sz max_power_of_two_leq(sz n)
{
    assert(n > 0); /* There is no power of two below or equal to 0. */
    if ((n & (n - 1)) == 0)
        return n; /* Already a power of two */
    return min_power_of_two_geq(n) / 2;
}

/* Returns the base-two exponent (order) of a given power-of-two length. */
static inline sz order_of(sz len)
{
    assert(len > 0 && (len & (len - 1)) == 0);
    int ord = 0;
    while (len >>= 1)
        ord++;
    return ord;
}

/* Returns the length corresponding to a given order as a power of two. */
static inline sz length_of_order(sz ord)
{
    assert(ord < SZ_WIDTH);
    return ((sz) 1LL) << ord;
}

struct buddy *buddy_init(struct byte_array ba, struct arena *arn)
{
    ALWAYS_ASSERT(ba.dat != NULL);
    ALWAYS_ASSERT(arn != NULL);
    ALWAYS_ASSERT(ba.len > 0);

    sz padding = (sz) (-(uptr) ba.dat & ((uptr) PAGE_SIZE - 1));
    if (padding > ba.len)
        return NULL;

    sz avail = ba.len - padding;
    if (avail <= 0)
        return NULL;

    /* The maximum usable length is the biggest power of two that is not greater
     * than 'avail'.
     */
    sz n_pages = max_power_of_two_leq(avail) / PAGE_SIZE;
    if (n_pages <= 0)
        return NULL;

    byte *base = ba.dat + padding;
    sz bitmap_len = (n_pages + BYTE_WIDTH - 1) / BYTE_WIDTH;

    if (bitmap_len <= 0 || bitmap_len > avail)
        return NULL;

    struct buddy *buddy = arena_alloc(arn, sizeof(*buddy));
    if (!buddy)
        return NULL;

    buddy->bitmap = byte_array_from_arena(bitmap_len, arn);
    if (buddy->bitmap.dat == NULL || buddy->bitmap.len <= 0)
        return NULL;

    for (sz i = 0; i < N_FREE_LISTS; i++)
        list_init(&buddy->avail[i].link);

    buddy->max_ord = order_of(n_pages);
    buddy->base = base;

    struct block *block = (struct block *) base;
    block->ord = buddy->max_ord;

    if (buddy->max_ord >= N_FREE_LISTS || buddy->max_ord < 0)
        return NULL;

    list_add(&buddy->avail[buddy->max_ord].link, &block->link);

    pr_debug(STR("Initialized buddy: base=0x%lx max_ord=%ld\n"), buddy->base,
             buddy->max_ord);
    return buddy;
}

static void set_bitmap(struct buddy *buddy, void *addr, sz ord, byte value)
{
    assert(buddy);

    sz bit_idx = ((byte *) addr - buddy->base) / PAGE_SIZE;
    sz bit_end = bit_idx + length_of_order(ord);
    assert(buddy->bitmap.len <= SZ_MAX / BYTE_WIDTH);
    assert(bit_end <= buddy->bitmap.len * BYTE_WIDTH);
    for (; bit_idx < bit_end; bit_idx++) {
        buddy->bitmap.dat[bit_idx / BYTE_WIDTH] &=
            ~(1 << (bit_idx % BYTE_WIDTH));
        buddy->bitmap.dat[bit_idx / BYTE_WIDTH] |= value
                                                   << (bit_idx % BYTE_WIDTH);
    }
}

static inline void set_avail(struct buddy *buddy, void *addr, sz ord)
{
    set_bitmap(buddy, addr, ord, 1);
}

static inline void set_not_avail(struct buddy *buddy, void *addr, sz ord)
{
    set_bitmap(buddy, addr, ord, 0);
}

static inline struct block *split_block(struct block *block, sz ord)
{
    /* Note that the memory is divided into units of page size. */
    sz len = length_of_order(ord);
    assert(len >= 0);
    assert(len <= SZ_MAX / PAGE_SIZE);
    sz offset = len * PAGE_SIZE;
    assert((ptr) block <= SZ_MAX - offset);
    return (struct block *) ((ptr) block + offset);
}

/* The size of the allocation in bytes is PAGE_SIZE * 2^req_ord. */
static void *buddy_alloc_raw(struct buddy *buddy, sz req_ord)
{
    if (!buddy || req_ord < 0 || req_ord > buddy->max_ord)
        return NULL;

    sz ord = req_ord;
    while (ord <= buddy->max_ord) {
        if (!list_empty(&buddy->avail[ord].link))
            break;
        ord++;
    }

    if (ord > buddy->max_ord) {
        printk(KERN_VERBOSE,
               STR("buddy: No block found for order %ld (max %ld), all "
                   "blocks too small\n"),
               req_ord, buddy->max_ord);
        return NULL;
    }

    struct block *ret =
        __container_of(buddy->avail[ord].link.next, struct block, link);

    set_not_avail(buddy, ret, ord);
    list_del(buddy->avail[ord].link.next);

    if (ord == req_ord) {
        printk(KERN_VERBOSE,
               STR("buddy: Found perfect fit: block=0x%08lx ord=%ld\n"),
               (uptr) ret, ord);
        return ret;
    }

    /* Split blocks until the order of the returned block matches the requested
     * order.
     */
    while (ord > req_ord) {
        ord--;
        struct block *rem = split_block(ret, ord);

        set_avail(buddy, rem, ord);
        rem->ord = ord;
        list_add(&buddy->avail[ord].link, &rem->link);
        printk(KERN_VERBOSE,
               STR("buddy: Split blocks: block=0x%08lx remainder=0x%08lx "
                   "ord=%ld\n"),
               (uptr) ret, (uptr) rem, ord);
    }

    printk(KERN_VERBOSE,
           STR("buddy: Found block after splitting: block=0x%08lx "
               "ord=%ld\n"),
           (uptr) ret, ord);
    return ret;
}

static inline struct block *get_buddy(struct buddy *buddy,
                                      struct block *block,
                                      sz ord)
{
    ptr base_offset = (ptr) block - (ptr) buddy->base;
    assert(length_of_order(ord + 1) <= SZ_MAX / PAGE_SIZE);
    sz mod = length_of_order(ord + 1) * PAGE_SIZE;
    assert(length_of_order(ord) <= SZ_MAX / PAGE_SIZE);
    sz halfway_offset = length_of_order(ord) * PAGE_SIZE;
    if (base_offset % mod == 0) {
        return (struct block *) ((ptr) block + halfway_offset);
    } else if (base_offset % mod == halfway_offset) {
        return (struct block *) ((ptr) block - halfway_offset);
    } else {
        crash("Invalid block pointer\n");
    }
}

static inline bool is_avail(struct buddy *buddy, void *addr, sz ord)
{
    assert(buddy);

    sz bit_idx = ((byte *) addr - buddy->base) / PAGE_SIZE;
    sz bit_end = bit_idx + length_of_order(ord);
    assert(buddy->bitmap.len <= SZ_MAX / BYTE_WIDTH);
    assert(bit_end <= buddy->bitmap.len * BYTE_WIDTH);

    for (; bit_idx < bit_end; bit_idx++) {
        sz byte_idx = bit_idx / BYTE_WIDTH;
        if ((buddy->bitmap.dat[byte_idx] & (1 << (bit_idx % BYTE_WIDTH))) == 0)
            return false;
    }

    return true;
}

static void buddy_free_raw(struct buddy *buddy, void *ptr, sz ord)
{
    if (!buddy || !ptr)
        return;
    if (ord < 0 || ord > buddy->max_ord)
        return;

    struct block *block = ptr;

    /* Detect double-free: the block being freed must not already be available.
     */
    if (is_avail(buddy, block, ord)) {
        crash("buddy_free_raw: double-free detected\n");
    }

    struct block *buddy_block = get_buddy(buddy, block, ord);

    printk(KERN_VERBOSE, STR("buddy: Freeing block: block=0x%08lx ord=%ld\n"),
           (uptr) block, ord);

    while (ord < buddy->max_ord && is_avail(buddy, buddy_block, ord)) {
        printk(KERN_VERBOSE,
               STR("buddy: Coalescing blocks: block=0x%08lx "
                   "buddy_block=0x%08lx ord=%ld\n"),
               (uptr) block, (uptr) buddy_block, ord);
        list_del(&buddy_block->link);
        ord++;
        if (buddy_block < block)
            block = buddy_block;
        buddy_block = get_buddy(buddy, block, ord);
    }

    block->ord = ord;
    set_avail(buddy, block, ord);
    list_add(&buddy->avail[ord].link, &block->link);
}

struct option_byte_array buddy_alloc(struct buddy *buddy, sz size)
{
    assert(buddy);
    assert(size > 0);
    /* 'order_of' expects the argument to be greater than 0. Round up to
     * 'PAGE_SIZE' so 'order_of' is not passed 0 if 'size' is below 'PAGE_SIZE'.
     */
    sz real_size = ALIGN_UP(size, PAGE_SIZE);
    /* The buddy system can only allocate memory in power-of-two-sized blocks so
     * the size must be rounded up to the nearest power of two. Because this
     * implementation deals in pages as the smallest unit, divide by the page
     * size to arrive at the number of blocks needed.
     */
    sz ord = order_of(min_power_of_two_geq(real_size) / PAGE_SIZE);
    void *mem = buddy_alloc_raw(buddy, ord);
    if (!mem)
        return option_byte_array_none();
    return option_byte_array_ok(byte_array_new(mem, size));
}

void buddy_free(struct buddy *buddy, struct byte_array ba)
{
    assert(buddy);
    assert(ba.len > 0);
    assert(ba.dat);

    /* See 'buddy_alloc' for comments. */
    sz real_size = ALIGN_UP(ba.len, PAGE_SIZE);
    sz ord = order_of(min_power_of_two_geq(real_size) / PAGE_SIZE);
    buddy_free_raw(buddy, ba.dat, ord);
}

struct buddy_stats buddy_stats_get(struct buddy *buddy)
{
    assert(buddy);
    struct buddy_stats stats = {
        .total_pages = length_of_order(buddy->max_ord),
        .free_pages = 0,
    };
    struct block *blk __unused;
    for (sz i = 0; i <= buddy->max_ord; i++) {
        if (list_empty(&buddy->avail[i].link))
            continue;
        list_for_each_entry_safe (&buddy->avail[i].link, blk, struct block,
                                  link) {
            stats.free_pages += length_of_order(i);
        }
    }
    return stats;
}

#include __INC_TEST(buddy)
