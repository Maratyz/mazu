...
static struct result_ipv4_addr_parsed rtcfg_parse_option_ip_addr(
    struct str *str)
{
    ...
    struct result_ipv4_addr_parsed pa_res =
        ipv4_addr_parse(str_new(str->dat, substr_len));
    if (pa_res.is_error)
        return pa_res;
...
static struct result rtcfg_parse_ip_addr_field(struct str *str,
                                               struct option_ipv4_addr *addr,
                                               struct option_ipv4_addr *mask)
{
    struct result_ipv4_addr_parsed res = rtcfg_parse_option_ip_addr(str);
    if (res.is_error)
        return result_error(res.code);

    struct ipv4_addr_parsed parsed = result_ipv4_addr_parsed_checked(res);
    *addr = option_ipv4_addr_ok(parsed.addr);
    if (mask)
        *mask = option_ipv4_addr_ok(parsed.mask);
    return result_ok();
}

static struct result rtcfg_parse(struct runtime_config *rtcfg,
                                 struct byte_view raw)
{
    ...
    while (str.len) {
        ...
        if (str_consume_prefix(&str, STR("host_ip"))) {
            struct result res =
                rtcfg_parse_ip_addr_field(&str, &rtcfg->host_ip, NULL);
            if (res.is_error)
                return res;
            continue;
        }
        ...
        if (str_consume_prefix(&str, STR("local_ip"))) {
            struct result res = rtcfg_parse_ip_addr_field(
                &str, &rtcfg->local_ip, &rtcfg->local_ip_mask);
            if (res.is_error)
                return res;
            continue;
        }
        ...
        if (str_consume_prefix(&str, STR("default_gateway_ip"))) {
            struct result res = rtcfg_parse_ip_addr_field(
                &str, &rtcfg->default_gateway_ip, NULL);
            if (res.is_error)
                return res;
            continue;
        }
                                               ...
                                               struct arena arn)
{
    ...
    struct result_ram_fs_node cfg_file_res =
        ram_fs_open(rfs->root, cfg_filename);
    ...
    struct byte_buf read_buf =
        byte_buf_from_array(byte_array_from_arena(cfg_file->data.len, &arn));
    ...
    /* The buffer was allocated to fit the entire file so this must hold. */
    assert(result_sz_checked(n_read_res) == cfg_file->data.len);

    struct option_byte_array rtcfg_mem_opt = kvalloc_alloc(
        sizeof(struct runtime_config), alignof(struct runtime_config));
    if (rtcfg_mem_opt.is_none)
        return result_runtime_config_error(ENOMEM);

    struct runtime_config *rtcfg =
        byte_array_ptr(option_byte_array_checked(rtcfg_mem_opt));
