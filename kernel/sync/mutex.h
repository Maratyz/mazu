 ...
 * the owner's priority is temporarily boosted to the highest waiter's
 * priority to prevent priority inversion.  On unlock, the owner's effective
 * priority is recomputed from all remaining held PI mutexes (falls back to
 * td_base_prio when no other PI mutexes are held).  Ownership is transferred
 * via direct handover, with no barging window.
 *
 * Single-level PI only (no transitive chain walking).
 *
...
/* Release the mutex.  Recomputes the caller's priority from any
 * remaining held PI mutexes and wakes the highest-priority waiter
 * via direct handover (ownership transfer under the lock).
 */
