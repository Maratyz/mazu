...
def djb2(data: bytes):
    ...
    for ch in data:
        hash_value = (
            ((hash_value << 5) + hash_value) + ch
        ) & 0xFFFFFFFFFFFFFFFF  # hash * 33 + c
    return hash_value


HEADER_SIZE = 24
INDEX_SIZE = 36
MAGIC_STRING = b"MAZU__AR"
HEADER_FORMAT = "<8sqq"
INDEX_FORMAT = "<QqqqI"
TEXT_ENC = "utf-8"
...
def extract_archive(archive_data):
    ...
    for _ in range(index_length):
        hash_value, file_offset, file_size_with_path, path_length, _ = struct.unpack(
            INDEX_FORMAT, archive_data[index_offset : index_offset + INDEX_SIZE]
        )
        index_offset += INDEX_SIZE

        if hash_value != djb2(
            archive_data[file_offset : file_offset + file_size_with_path]
        ):
            raise ValueError("Data doesn't match hash")

        file_path = archive_data[file_offset : file_offset + path_length].decode(
            TEXT_ENC
        )
        file_data = archive_data[
            file_offset + path_length : file_offset + file_size_with_path
        ]

        # Removing the extra slash at the beginning (see comment create_archive).
        file_info.append(
            (file_path.strip("/"), file_size_with_path - path_length, file_data)
        )
